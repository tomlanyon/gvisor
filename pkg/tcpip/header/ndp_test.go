...
func TestNDPNeighborAdvert(t *testing.T) {
	...
	// Make sure the address got updated in the backing buffer.
	if got := tcpip.Address(b[ndpNATargetAddessOffset:][:IPv6AddressSize]); got != addr2 {
		t.Fatalf("got targetaddress buffer = %s, want %s", got, addr2)
	}
...
// TestNDPRouterAdvert tests the functions of NDPRouterAdvert.
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := tll.EthernetAddress(); got != test.expected {
				t.Fatalf("got tll.EthernetAddress = %s, want = %s", got, test.expected)
			}
		})
	}
...
func TestNDPTargetLinkLayerAddressOptionSerialize(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			it, ok := opts.Iter(true)
			if !ok {
				t.Fatalf("got Iter = (_, false), want = (_, true)")
			}

			if len(test.expectedBuf) > 0 {
				next, done, ok := it.Next()
				if !ok {
					t.Fatalf("got Next = (_, _, true), want = (_, _, false)")
				}
				if done {
					t.Fatalf("got Next = (_, true, _), want = (_, false, _)")
				}
				...
				if got, want := next.Body(), test.expectedBuf[2:]; !bytes.Equal(got, want) {
					t.Fatalf("got Next = (%x, _, _), want = (%x, _, _)", got, want)
				}

				tll := NDPTargetLinkLayerAddressOption(next.Body())

				if got, want := tll.EthernetAddress(), tcpip.LinkAddress(test.expectedBuf[2:][:EthernetAddressSize]); got != want {
					t.Fatalf("got tll.MACAddress = %s, want = %s", got, want)
				}
			}
			...
			if !done {
				t.Fatalf("got Next = (_, false, _), want = (_, true, _)")
			}
			if next != nil {
				t.Fatalf("got Next = (%x, _, _), want = (nil, _, _)", next)
			}
		})
	}
}
...
func TestNDPOptionsIterCheck(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, ok := opts.Iter(true); ok != test.expected {
				t.Fatalf("got Iter(true) = (_, %t), want = (_, %t)", ok, test.expected)
			}

			// test.buf may be malformed but we chose not to check
			// the iterator so it must return true.
			if _, ok := opts.Iter(false); !ok {
				t.Fatalf("got Iter(false) = (_, false), want = (_, true)")
			}
		})
	}
}
...
func TestNDPOptionsIter(t *testing.T) {
	...
	if got, want := next.Body(), buf[2:][:6]; !bytes.Equal(got, want) {
		t.Fatalf("got Next = (%x, _, _), want = (%x, _, _)", got, want)
	}
	if got := next.Type(); got != NDPTargetLinkLayerAddressOptionType {
		t.Fatalf("got Type = %d, want = %d", got, NDPTargetLinkLayerAddressOptionType)
	...
	if got, want := next.Body(), buf[26:][:30]; !bytes.Equal(got, want) {
		t.Fatalf("got Next = (%x, _, _), want = (%x, _, _)", got, want)
	}
	if got := next.Type(); got != NDPPrefixInformationType {
		t.Fatalf("got Type = %d, want = %d", got, NDPPrefixInformationType)
