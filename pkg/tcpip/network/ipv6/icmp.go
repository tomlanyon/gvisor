...
func (e *endpoint) handleICMP(r *stack.Route, netHeader buffer.View, vv buffer.VectorisedView) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		ns := header.NDPNeighborSolicit(h.Body())
		...
		if err != nil {
			...
			log.Printf("ndpns: Invalid NICID for route = %d; err = %s", rxNICID, err)
			return
		}
		...
		if isTentative {
			...
			if iph.SourceAddress() == header.IPv6Any {
				log.Printf("ndpns: Duplicate tentative address (%s) is detected on NIC(%d)", targetAddr, rxNICID)
			...
			// NIC any further.
			log.Printf("ndpns: Ending processing as target address (%s) is tentative on NIC(%d)", targetAddr, rxNICID)
			return
		}
	...
	case header.ICMPv6NeighborAdvert:
		...
		na := header.NDPNeighborAdvert(h.Body())
		...
		if err != nil {
			...
			//
			// TODO(b/141002840): Handle this better?
			log.Printf("ndpna: Invalid NICID for route = %d; err = %s", rxNICID, err)
			return
		}
		...
		if isTentative {
			...
			// the NDP NA.
			log.Printf("ndpna: Duplicate address (%s) is detected on NIC(%d)", targetAddr, rxNICID)
			stack.DupTentativeAddrDetected(rxNICID, targetAddr)
			return
		}
