...
// when address' kind gets "promoted" to permanent from permanentExpired.
	...
	for _, pi := range pebs {
		for _, ps := range pebs {
			t.Run(fmt.Sprintf("%d-to-%d", pi, ps), func(t *testing.T) {
				...
				// Take a route to through the address so its
				// ref count gets incremented and does not
				// actually get deleted when RemoveAddress is
				// called below. This is because we want to test
				// that a new peb is respected when an address
				// gets "promoted" to permanent from a
				...
				if ps == stack.FirstPrimaryEndpoint {
					expectedList = []tcpip.Address{
						"\x01",
						"\x03",
					}
				} else if ps == stack.CanBePrimaryEndpoint {
					expectedList = []tcpip.Address{
						"\x03",
						"\x01",
					}
				...
				if !reflect.DeepEqual(primaryAddrs, expectedList) {
