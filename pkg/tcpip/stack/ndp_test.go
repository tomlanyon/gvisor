...
// raBuf returns a valid NDP Router Advertisement IPv6 packet in a
// buffer.Prependable to be used for the following tests:
//  - TestNoRouterDiscoveryUpdateRoutingTable
//  - TestRouterDiscoveryUpdateRoutingTable
//  - TestRouterDiscoveryMaxRouters
//
// Note, raBuf does not populate any of the RA fields other than the
// Router Lifetime.
func raBuf(ip tcpip.Address, rl uint16) buffer.Prependable {
...
// TestNoRouterDiscoveryUpdateRoutingTable tests that the routing table will not
// get updated due to incoming Router Advertisements if configured not to do so.
func TestNoRouterDiscoveryUpdateRoutingTable(t *testing.T) {
	for i := 0; i < 15; i++ {
		handle := i&1 != 0
		discover := i&2 != 0
		autoUpdate := i&4 != 0
		...
		t.Run(fmt.Sprintf("HandleRAs(%t), DiscoverDefaultRouters(%t), AutoUpdateRoutingTable(%d), Forwarding(%t)", handle, discover, autoUpdate, forwarding), func(t *testing.T) {
			e := channel.New(10, 1280, linkAddr1)
			s := stack.New([]string{ipv6.ProtocolName}, nil, stack.Options{
				NDPConfigs: stack.NDPConfigurations{
					HandleRAs:              handle,
					DiscoverDefaultRouters: discover,
					AutoUpdateRoutingTable: autoUpdate,
				},
			})
			...
			e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr2, 1000).View().ToVectorisedView())
...
// TestRouterDiscoveryUpdateRoutingTable tests that the routing table will be
// updated due to incoming Router Advertisements with non-zero lifetimes.
func TestRouterDiscoveryUpdateRoutingTable(t *testing.T) {
	e := channel.New(10, 1280, linkAddr1)
	s := stack.New([]string{ipv6.ProtocolName}, nil, stack.Options{
		NDPConfigs: stack.NDPConfigurations{
			HandleRAs:              true,
			DiscoverDefaultRouters: true,
			AutoUpdateRoutingTable: true,
		},
	})
	...
	// Rx an RA from lladdr2 with zero lifetime.
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr2, 0).View().ToVectorisedView())

	// Should still not have any routes since the RA had a lifetime = 0.
	if got := len(s.GetRouteTable()); got != 0 {
		t.Fatalf("got len(s.GetRouteTable()) = %d, want = 0", got)
	}
	...
	if got, want := s.GetRouteTable(), []tcpip.Route{{header.IPv6EmptySubnet, llAddr2, 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Rx an RA from another router (lladdr3) with non-zero lifetime.
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr3, 6).View().ToVectorisedView())

	// Should have default routes through the discovered routers.
	if got, want := s.GetRouteTable(), []tcpip.Route{{header.IPv6EmptySubnet, llAddr2, 1}, {header.IPv6EmptySubnet, llAddr3, 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Rx an RA from lladdr2 with lesser lifetime.
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr2, 2).View().ToVectorisedView())
	...
	// lifetime.
	//
	// TODO(b/141569273): Use events from the stack to know when a
	//                    discovered router gets invalidated.
	time.Sleep(3 * time.Second)

	// Should no longer have the default route through lladdr2.
	if got, want := s.GetRouteTable(), []tcpip.Route{{header.IPv6EmptySubnet, llAddr3, 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}
	...
	if got, want := s.GetRouteTable(), []tcpip.Route{{header.IPv6EmptySubnet, llAddr3, 1}, {header.IPv6EmptySubnet, llAddr2, 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}
...
func TestRouterDiscoveryMaxRouters(t *testing.T) {
	...
	for i := 1; i <= stack.MaxDiscoveredDefaultRouters+2; i++ {
		...
		e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr, 5).View().ToVectorisedView())
	...
	if got := s.GetRouteTable(); !reflect.DeepEqual(got, expectedRt[:]) {
