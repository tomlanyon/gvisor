	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for i := uint8(0); i < test.dupAddrDetectTransmits; i++ {
				...
				vv := p.Header.ToVectorisedView()

				// Check IPv6 header.
				h := header.IPv6(vv.First())
				if !h.IsValid(vv.Size()) {
					t.Fatalf("got IsValid(_) = false, want = true")
				}
				vv.TrimFront(header.IPv6MinimumSize)
				vv.CapLength(int(h.PayloadLength()))
				if transProto := h.TransportProtocol(); transProto != header.ICMPv6ProtocolNumber {
					t.Fatalf("got h.TransportProtocol(_) = %d, want = %d", transProto, header.ICMPv6ProtocolNumber)
				}

				// Check ICMPv6 header.
				icmpV := vv.First()
				if got := len(icmpV); got < header.ICMPv6MinimumSize {
					t.Fatalf("got len(icmpV) = %d, want >= %d", got, header.ICMPv6MinimumSize)
				}
				icmpH := header.ICMPv6(icmpV)
				if got := icmpH.Type(); got != header.ICMPv6NeighborSolicit {
					t.Fatalf("got Type() = %d, want = %d", got, header.ICMPv6NeighborSolicit)
				}
				if got := icmpH.Code(); got != 0 {
					t.Fatalf("got Code() = %d, want = 0", got)
				}
				icmpB := icmpH.Body()
				if got := len(icmpB); got < header.NDPNSMinimumSize {
					t.Fatalf("got len(icmpB) = %d, want >= %d", got, header.NDPNSMinimumSize)
				}

				// Check Neighbor Solicitation message.
				ns := header.NDPNeighborSolicit(icmpB)
				if got := ns.TargetAddress(); got != addr1 {
					t.Fatalf("got TargetAddress() = %s, want = %s", got, addr1)
				}
			}
		})
	}
...
func TestDADFail(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			opts := stack.Options{
				NDPConfigs: stack.MakeDefaultNDPConfigurations(),
			}
...
func TestDADStop(t *testing.T) {
	...
	// Should not have an address still since we removed it and DAD should
	// have stopped.
	if addr, err := s.GetMainNICAddress(1, header.IPv6ProtocolNumber); err != tcpip.ErrNoLinkAddress {
		t.Fatalf("stack.GetMainNICAddress(_, _) = %s, %v", addr, err)
	}
