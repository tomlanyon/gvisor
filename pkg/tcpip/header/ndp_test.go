...
// TestNDPPrefixInformationOption tests the field getters of an
...
func TestNDPPrefixInformationOption(t *testing.T) {
	...
	if !pi.OnLinkFlag() {
		t.Fatalf("got OnLinkFlag = false, want = true")
	}

	// Test getting the Autonomous Address Configuration bit-field.
	if pi.AutonomousAddressConfigurationFlag() {
		t.Fatalf("got AutonomousAddressConfigurationFlag = true, want = false")
	}
	...
	if got, want := pi.Prefix(), tcpip.Address("\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18"); got != want {
		t.Fatalf("got Prefix = %q, want = %q", got, want)
	}
}

// TestNDPPrefixInformationOptionSerialize tests serializing an
// NDPPrefixInformation.
func TestNDPPrefixInformationOptionSerialize(t *testing.T) {
	b := []byte{
		43, 255,
		1, 2, 3, 4,
		5, 6, 7, 8,
	...
	if !bytes.Equal(targetBuf, expectedBuf) {
		t.Fatalf("got targetBuf = %q, want = %q", targetBuf, expectedBuf)
	}
}
