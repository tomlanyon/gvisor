...
func TestNICAutoGenAddr(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			s := stack.New([]string{ipv6.ProtocolName}, nil, opts)
			...
			if test.shouldGen {
				...
				if want := header.LinkLocalAddr(test.linkAddr); addr.Address != want {
				...
				if addr, err := s.GetMainNICAddress(1, header.IPv6ProtocolNumber); err != tcpip.ErrNoLinkAddress {
					t.Fatalf("got stack.GetMainNICAddress(_, _) = (%s, %v), want = (nil, tcpip.ErrNoLinkAddress)", addr, err)
				}
			}
		})
	}
...
func TestNICAutoGenAddrDoesDAD(t *testing.T) {
	...
	if want := header.LinkLocalAddr(linkAddr1); addr.Address != want {
		t.Fatalf("got stack.GetMainNICAddress(_, _) = %s, want = %s", addr, want)
	}
}
